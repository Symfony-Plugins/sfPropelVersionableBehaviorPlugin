...
abstract class BaseResourceVersionPeer {
	...
	public static function doCountJoinPost(Criteria $criteria, $distinct = false, $con = null)
	{
		...
		$criteria->addJoin(ResourceVersionPeer::RESOURCE_UUID, PostPeer::UUID);
	...
	public static function doSelectJoinPost(Criteria $c, $con = null)
	{
		...
		$startcol = (ResourceVersionPeer::NUM_COLUMNS - ResourceVersionPeer::NUM_LAZY_LOAD_COLUMNS) + 1;
		PostPeer::addSelectColumns($c);

		$c->addJoin(ResourceVersionPeer::RESOURCE_UUID, PostPeer::UUID);
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				$temp_obj2 = $temp_obj1->getPost(); 				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
										$temp_obj2->addResourceVersion($obj1); 					break;
				}
			}
			if ($newObject) {
				$obj2->initResourceVersions();
				$obj2->addResourceVersion($obj1); 			}
			$results[] = $obj1;
		}
		return $results;
	}
